...
(function (win, $, und) {
	...
	const
		DisplayDict = { 'Luogu': '洛谷', 'SPOJ': 'Sphere OJ', 'rg': 'Regulus OJ', 'Local': '本地', 'NowCoder': '牛客', 'zr': '正睿', 'Unknown': '其他的 OJ', 'HaoBa': 'HJJ'},
		...
		SiteDict = {
			...
			'mx': [/^([TU]?)(\d+)$/, (x, y) => `http://47.94.97.204/contest/${x}/problem/${x.charCodeAt()-97+1}`],
			...
			'rg': [/^([TU]?)(\d+)$/, (x, y) => `http://39.106.65.172/d/Rigel/p/${x || 'P'}${y}`],
			'ac': [/^(\d+)$/, getAtCoderID],
